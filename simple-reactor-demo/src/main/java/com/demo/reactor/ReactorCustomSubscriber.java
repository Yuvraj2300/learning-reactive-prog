...
class CustomSubscriber extends BaseSubscriber<String> {
	@Override
	protected void hookOnSubscribe(Subscription subscription) {
		...
		/*
		 * for(int index=0; index<6;index++) { try { Thread.sleep(1000); } catch
		 * (InterruptedException e) { e.printStackTrace(); } subscription.request(1); }
		 */
